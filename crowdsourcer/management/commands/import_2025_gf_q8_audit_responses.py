...
class Command(BaseImporter):
    ...
    def get_gf_8_answer(self, q, row, value):
        if value == 0 or value == "No" or pd.isna(value):
            ...
            try:
                ...
                self.print_error(
                    f"issue with {row['public_body']} : sixty: {sixty_percent_staff}, total_staff: {total_staff}"
                )
                percent_staff = 0
    ...
    def get_standard_defaults(self, name, row):
        ...
        if row.get("public_notes") and not pd.isna(row["public_notes"]):
            defaults["evidence"] = row["public_notes"]

        return defaults

    def get_defaults_for_q(self, name, q, row, details):
        MINIMUM_CRITERIA_MET = 15
        ...
        option = self.get_gf_8_answer(q, row, value)
        if option is None:
            option = self.get_option_for_question(q, "Evidence doesn't meet criteria")
    ...
    def get_authority(self, authority, council_lookup):
        ...
        try:
            ...
            self.warnings.append(
                f"no such authority: {orig_authority} ({authority} [{gss}])"
            )
            return None
    ...
    def process_sheet(self, sheet_map, council_lookup, rt, u, ms, combined=False):
        ...
        for sheet, name in sheets.items():
            ...
            for _, row in df.iterrows():
                ...
                if not defaults.get("evidence") or not defaults.get("private_notes"):
                    try:
                        ...
                        self.print_error(
                            f"problem geting prev_response for {authority}: {e}"
                        )
                        continue

                    if not defaults.get("notes"):
                        self.print_debug(f"using prev notes for {authority}")
                        defaults["evidence"] = prev_r.evidence
                ...
                try:
                    ...
                    self.print_error(f"problem creating responsefor {authority}: {e}")
                    continue
