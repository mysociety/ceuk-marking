...
class Command(BaseImporter):
    ...
    def get_bh_2_answer(self, q, row, value):
        ...
        if row["Does the council have a 100% renewable electricity tariff?"] == "Yes":
            green_energy = True
        ...
        if green_energy:
            options.append(
                Option.objects.get(question=q, description="Yes, 100% Green tariff")
            )
        elif greater_than_20:
            options.append(
                Option.objects.get(
                    question=q,
                    description="Yes, council generates 20% or more of its own energy through other renewable energy production",
                )
            )
        else:
            options.append(
